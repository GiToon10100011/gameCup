...
| 첫 사이클 길이가 28일이 아니라 29일로 나옴 | Linear가 cadence를 주(week) 경계에 정렬하며 첫 구간만 보정되는 것으로 관찰됨(실측). 2번째 사이클부터는 정확히 28일 | 정상 동작으로 간주하고 별도 조치 불필요. **참고:** Linear 공식 문서([use-cycles](https://linear.app/docs/use-cycles), [update-cycles](https://linear.app/docs/update-cycles))는 사이클 길이를 "주 단위 동일 duration"·"시작 요일 12:01 AM"으로만 규정하며 첫 사이클을 28일로 **강제**하는 규칙은 명시하지 않는다. 표시 일수는 주 경계·타임존·생성 방식에 따라 달라질 수 있으므로, 28일 첫 사이클이 꼭 필요하면 강제 설정 대신 Cycles 뷰에서 실제 생성된 start/end 날짜를 확인해 **실측 기반으로 재조정**한다 |
